...
export const VoiceIntakeScreen: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col justify-center max-w-3xl mx-auto w-full px-4 sm:px-6 py-6 animate-fadeIn">
      ...
      <div className="bg-slate-900 rounded-2xl p-5 sm:p-6 text-white shadow-md mb-6 relative">
        <div className="flex items-start space-x-3.5">
          <div className="w-10 h-10 rounded-xl bg-teal-500/20 border border-teal-400/30 text-teal-300 flex items-center justify-center font-bold shrink-0">
            <Volume2 className="w-5 h-5" />
          </div>
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-semibold uppercase tracking-wider text-teal-400">
                Intake Question ({lang.name})
              </span>
              <button
                onClick={() => speakText(localizedAiQuestion, currentLanguage)}
                className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-teal-300 text-xs flex items-center space-x-1 transition-colors"
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm text-center flex flex-col items-center relative mb-5">
        ...
        <div className="mb-5">
          {micState === 'IDLE' && (
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-xs font-semibold">
              ...
              <span>Tap microphone to record symptoms</span>
            </div>
          )}
          {micState === 'LISTENING' && (
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-red-50 text-red-700 border border-red-200 text-xs font-semibold animate-pulse">
              <span className="w-2 h-2 rounded-full bg-red-600"></span>
              <span>Listening... Speak clearly</span>
            </div>
          )}
          {micState === 'PROCESSING' && (
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-teal-50 text-teal-700 border border-teal-200 text-xs font-semibold">
        ...
        <div className="relative mb-5">
          <button
            ...
            className={`relative w-24 h-24 sm:w-28 sm:h-28 rounded-full flex items-center justify-center shadow-md transition-all cursor-pointer ${
              micState === 'LISTENING'
        ...
        {micState === 'LISTENING' && (
          <div className="flex items-center justify-center space-x-1.5 h-8 mb-4">
            <span className="w-1.5 bg-teal-600 rounded-full animate-soundwave-1"></span>
            <span className="w-1.5 bg-teal-600 rounded-full animate-soundwave-2"></span>
            <span className="w-1.5 bg-teal-600 rounded-full animate-soundwave-3"></span>
            <span className="w-1.5 bg-teal-600 rounded-full animate-soundwave-4"></span>
            <span className="w-1.5 bg-teal-600 rounded-full animate-soundwave-5"></span>
          </div>
        )}

        {transcription ? (
          <div className="w-full max-w-lg bg-slate-50 border border-teal-200 rounded-xl p-4 mb-5 text-left animate-fadeIn">
            ...
            <div className="mt-3 pt-3 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-2.5">
              ...
              <div className="flex items-center space-x-2 w-full sm:w-auto">
                ...
                <button
                  ...
                  className="flex-1 sm:flex-initial px-4 py-1.5 rounded-lg bg-teal-600 hover:bg-teal-700 text-white font-semibold text-xs shadow-sm flex items-center justify-center space-x-1 transition-all"
                >
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-xs flex items-center space-x-1.5 transition-colors"
          >
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 font-medium text-xs flex items-center space-x-1.5 transition-colors"
        ...
        {isTypingMode && (
          <form onSubmit={handleManualSubmit} className="w-full max-w-lg mt-4 p-3 rounded-xl bg-slate-50 border border-slate-200 text-left animate-fadeIn">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg border border-slate-300 focus:outline-none focus:border-teal-600 text-xs text-slate-900"
              />
              <button
                type="submit"
                className="px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white font-semibold text-xs flex items-center space-x-1 shadow-sm"
              >
